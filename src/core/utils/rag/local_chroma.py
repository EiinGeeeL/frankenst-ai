...
from core.utils.rag.langchain_unstructured_indexer import LangChainMultiVectorDocumentIndexer


LOCAL_CHROMA_COLLECTION_NAME = "local_pokemon_collection"
LOCAL_CHROMA_DIRECTORY = Path("artifacts/.chromadb")
LOCAL_DOCSTORE_DIRECTORY = Path("artifacts/.docstore")
LOCAL_RAG_DOCS_DIRECTORY = Path("artifacts/rag_docs")
LOCAL_DOC_ID_KEY = "doc_id"
LOCAL_MIN_IMAGE_SIZE = (128, 128)


def create_local_docstore(docstore_directory: str | Path = LOCAL_DOCSTORE_DIRECTORY) -> BaseStore:
    """Create or open the persistent local docstore used by the local retriever."""

    directory = Path(docstore_directory)
...
def _docstore_has_data(docstore_directory: str | Path) -> bool:
    directory = Path(docstore_directory)
    return directory.exists() and any(directory.iterdir())


def _get_pdf_paths(rag_docs_directory: str | Path) -> list[Path]:
    directory = Path(rag_docs_directory)
...
def bootstrap_local_vectorstore(
    ...
    persist_directory: str | Path = LOCAL_CHROMA_DIRECTORY,
    docstore_directory: str | Path = LOCAL_DOCSTORE_DIRECTORY,
    rag_docs_directory: str | Path = LOCAL_RAG_DOCS_DIRECTORY,
...
def get_or_create_local_vectorstore(
    ...
    raise RuntimeError(
        "Local Chroma persistence is inconsistent. Keep both artifacts/.chromadb and artifacts/.docstore in sync "
        "or remove them to bootstrap the local collection again."
    )