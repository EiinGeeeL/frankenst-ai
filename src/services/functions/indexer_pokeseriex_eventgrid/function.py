...
def main(event: func.EventGridEvent):
    ...
    if index_name in event.subject:
        try:
            Orchestrator.check_index(index_name)
        except Exception as e:
        ...
        if event.event_type == "Microsoft.Storage.BlobCreated":
            ...
            try:
                Orchestrator.document_indexing(index_name, event.subject)
            except Exception as e:
            ...
            try:
                Orchestrator.delete_document_by_filename(index_name, event.subject)
            except Exception as e:
